...
public class CreateData {
	...
	public int create2016Data() {
		...
		driverRepo.save(new Driver("Valtteri Bottas", 77, carRepo.findByName("Mercedes").get(0), 35));
		...
		driverRepo.save(new Driver("Lance Stroll", 18, carRepo.findByName("Williams").get(0), 27));

		driverRepo.save(new Driver("Sergio Perez", 11, carRepo.findByName("Force India").get(0), 20));
		driverRepo.save(new Driver("Esteban Ocon", 31, carRepo.findByName("Force India").get(0), 19));
		...
		driverRepo.save(new Driver("Stoffel Vandoorne", 47, carRepo.findByName("McLaren").get(0), 14));
		...
		driverRepo.save(new Driver("Kevin Magnussen", 20, carRepo.findByName("Haas F1 Team").get(0), 10));
