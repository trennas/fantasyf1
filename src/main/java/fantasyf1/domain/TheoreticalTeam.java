...
public class TheoreticalTeam extends AutogeneratedPK {
	...
	public TheoreticalTeam copy() {
		TheoreticalTeam t = new TheoreticalTeam();
		t.setName(name+"_copy");
		t.setPoints((long)0);
		if(drivers != null) {
			t.setDrivers(new ArrayList<>());
			for(MinimalTeamComponent driver : drivers) {
				t.getDrivers().add(driver);
			}
		}
		t.setCar(car);
		t.setEngine(engine);
